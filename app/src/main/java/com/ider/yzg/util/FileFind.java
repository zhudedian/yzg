...
public class FileFind {
    ...
    private static void addFile(File f,List<BoxFile> uploadFiles){
        if (f.isDirectory()) {
            ...
            uploadFiles.add(new BoxFile(3, f.getName(), FileUtil.getLTime(f),FileUtil.getLSize(f), f.getPath()));
        }else if (FileUtil.getFileType(f).equals(FileUtil.str_image_type)&&MyData.disPlayMode.equals(MyData.NORMAL)){
            uploadFiles.add(new BoxFile(4, f.getName(), FileUtil.getLTime(f),FileUtil.getLSize(f), f.getPath()));
        }else if (FileUtil.getFileType(f).equals(FileUtil.str_apk_type)&&MyData.disPlayMode.equals(MyData.NORMAL)){
            uploadFiles.add(new BoxFile(5, f.getName(), FileUtil.getLTime(f),FileUtil.getLSize(f), f.getPath()));
        }else if (FileUtil.getFileType(f).equals(FileUtil.str_zip_type)&&MyData.disPlayMode.equals(MyData.NORMAL)){
            uploadFiles.add(new BoxFile(6, f.getName(),FileUtil.getLTime(f), FileUtil.getLSize(f), f.getPath()));
        }else if (FileUtil.getFileType(f).equals(FileUtil.str_pdf_type)&&MyData.disPlayMode.equals(MyData.NORMAL)){
            uploadFiles.add(new BoxFile(7, f.getName(),FileUtil.getLTime(f), FileUtil.getLSize(f), f.getPath()));
        }else if (FileUtil.getFileType(f).equals(FileUtil.str_txt_type)&&MyData.disPlayMode.equals(MyData.NORMAL)){
            uploadFiles.add(new BoxFile(8, f.getName(),FileUtil.getLTime(f), FileUtil.getLSize(f), f.getPath()));
        }else {
            if (MyData.disPlayMode.equals(MyData.NORMAL)) {
                uploadFiles.add(new BoxFile(9, f.getName(), FileUtil.getLTime(f), FileUtil.getLSize(f), f.getPath()));
            }else {
                MyData.hideFiles.add(new BoxFile(9, f.getName(), FileUtil.getLTime(f), FileUtil.getLSize(f), f.getPath()));
            }
        }
    }
}
